...
def main():
    ...
    scored_df.write.mode("overwrite").parquet(os.path.join(PROCESSED_DATA_PATH, "scored_data.parquet"))
    
    # 5. Contagion Matrix
    print("Building Contagion Matrix (Pearson Correlation)...")
    corr_matrix = build_contagion_matrix(scored_df)
    
    # Save locally first, then move to HDFS to bypass PyArrow libjvm issues
    local_csv_path = "./contagion_matrix.csv"
    corr_matrix.to_csv(local_csv_path)
    
    # Use hdfs command line to upload
    print(f"Uploading contagion matrix to HDFS...")
    import subprocess
    subprocess.run(["hdfs", "dfs", "-rm", "-f", os.path.join(PROCESSED_DATA_PATH, "contagion_matrix.csv")], check=False)
    ...
    forecast_df.write.mode("overwrite").parquet(os.path.join(PROCESSED_DATA_PATH, "forecast_data.parquet"))
